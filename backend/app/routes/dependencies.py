...
oauth2_scheme = OAuth2PasswordBearer(tokenUrl="/auth/login") # This where users get token if they dont have one
...
async def store_user_refresh_token(db: AsyncSession, user_id: uuid.UUID, refresh_token:str):
...
async def create_access_token(user: User) -> str:
    access_expire_time = datetime.now(timezone.utc) + timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    to_encode_access = {"sub": str(user.user_id), "exp" : access_expire_time, "type" : "access"}
...
async def create_full_token(user: User) -> dict:
    ...
    refresh_expire_time = datetime.now(timezone.utc) + timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS)
    to_encode_refres = {"sub": str(user.user_id), "exp" : refresh_expire_time, "type" : "refresh"}
    if SECRET_KEY is None:
        raise ValueError("SECRET_KEY not found")
    encoded_jwt_refresh = jwt.encode(to_encode_refres, SECRET_KEY, algorithm=ALGORITHM)
# Check refresh token exist in db or not
    return {"access_token": encoded_jwt_access, "refresh_token": encoded_jwt_refresh, "token_type": "bearer"}
def verify_refresh_token(token: str) -> uuid.UUID:
    ...
    try:
        ...
        if not user_id or token_type != "refresh":
            raise HTTPException(status_code=401, detail="Invalid token type or missing subject")
...
async def verify_uniqueness(db, model, schema_in, exclude_id=None):
    ...
    unique_columns = [col.key for col in mapper.attrs if hasattr(col, 'columns') and col.columns[0].unique] 
