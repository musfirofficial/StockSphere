...
from app.models import Supplier, StockAlert, AlertStatus, PurchaseOrder, PurchaseOrderItem, POType
from app.schemas.purchaseorder import PurchaseOrderCreate, PurchaseOrderItemCreate, PurchaseOrderItemUpdate
...
async def get_purchase_order_by_id(db: AsyncSession, order_id: uuid.UUID) -> PurchaseOrder | None:
    result = await db.execute(select(PurchaseOrder).filter(PurchaseOrder.po_id == order_id))
    return result.scalar_one_or_none()

# ----------------------- CRUD for get all purchase orders ---------------------- #
async def get_all_purchase_orders(db: AsyncSession) -> Sequence[PurchaseOrder] | None:
...
async def update_purchase_order_items_bulk(
    db: AsyncSession,
    po_id: uuid.UUID,
    updates: list[PurchaseOrderItemUpdate]
) -> int:
...
async def delete_purchase_order_item(db: AsyncSession, poi_id: uuid.UUID):
    await db.execute(
        ...
        .where(PurchaseOrderItem.poi_id == poi_id)
    )
    await db.commit()

# --------------------- Get purchase order item by poi id -------------------- #
async def get_purchase_order_item_by_poi_id(db: AsyncSession, poi_id: uuid.UUID) -> PurchaseOrderItem | None:
    result = await db.execute(select(PurchaseOrderItem).filter(PurchaseOrderItem.poi_id == poi_id))
    return result.scalar_one_or_none()

# --------------------- Get purchase order items by po_id -------------------- #
async def get_purchase_order_items_by_po_id(db: AsyncSession, po_id: uuid.UUID) -> Sequence[PurchaseOrderItem] | None:
    result = await db.execute(select(PurchaseOrderItem).filter(PurchaseOrderItem.po_id == po_id))
    return result.scalars().all()
...
async def get_existing_draft_po(db: AsyncSession, supplier_id: uuid.UUID) -> PurchaseOrder | None:
    result = await db.execute(
...
async def get_purchase_order_with_poi(db: AsyncSession, po_id: uuid.UUID) -> PurchaseOrder | None:
    po_result = await db.execute(
        select(PurchaseOrder)
        .where(PurchaseOrder.po_id == po_id)
        .options(selectinload(PurchaseOrder.purchaseorderitems).selectinload(PurchaseOrderItem.item))
    )
