...
sequenceDiagram
    ...
    AuthAPI-->>Cookie: Set-Cookie: refresh_token=...; HttpOnly; SameSite=Lax; Secure
    AuthAPI-->>Client: 200 OK { access_token, user }

    Note over Client, Cookie: Authenticated API Request Flow
    Client->>AuthAPI: GET /items/ [Authorization: Bearer <access_token>]
    AuthAPI-->>Client: 200 OK [Data Payload]

    Note over Client, Cookie: Token Expiration & Silent Refresh Flow
    Client->>AuthAPI: GET /items/ [Expired Access Token]
    AuthAPI-->>Client: 401 Unauthorized
    Client->>AuthAPI: POST /auth/refresh-token (Cookie sent automatically)
    AuthAPI->>JWT: Validate Refresh Token & Generate Fresh Access Token
    AuthAPI-->>Client: 200 OK { access_token: fresh_token }
    Client->>AuthAPI: GET /items/ [Fresh Access Token]
    AuthAPI-->>Client: 200 OK [Data Payload]
```
